...
### ✅ 작업 5 — S1 "오늘의 우주" (F1) end-to-end ⭐ Turn 3 목표 달성
- `src/lib/{types,api}.ts`, `src/components/{CosmicCard,Loader}.tsx`,
  `src/pages/Home.tsx`, `src/App.tsx`
- 진입 시 오늘 APOD 자동 로드 → 그 날짜로 essay 스토리 생성 → 카드 표시
- 로딩 스켈레톤 / 에러+재시도 / 폴백 안내 배너 / NASA 크레딧·독립프로젝트 표기
- 우주 다크 + 글래스 카드(명세 8장)
- **검증**: 오늘(2026-06-26) "Milky Way Urban Style" 이미지 + 한국어 에세이 생성 정상

## 2026-06-27 — 작업 6: 생일의 우주 루프 (F2·F3) + 디자인 토큰

### 6-A. 디자인 토큰 (Stargazer 방법론 → 우주 테마)
- tailwind.config: 그림자 e1/e2/e3·glow, radius(card/control), motion easing,
  keyframes(cardIn/riseIn/twinkle/shimmer)
- index.css: `.glass`/`.glass-strong` 유틸, `.num`(tabular-nums), reduced-motion off
- Home/CosmicCard 토큰 기반으로 통일

### 6-B. S2 생일 입력 (F2)
- pages/Birthday.tsx + components/DateField.tsx (native date, min 1995-06-16/max today)
- 이름 입력(선택), 유효성 검사, App.tsx 뷰 상태기(home→input→result)
- S1 "내 생일의 우주 보기" 버튼 실제 연결

### 6-C. S3 결과 + 톤 선택 (F3)
- pages/Result.tsx + components/ToneToggle.tsx (essay/fortune/poem 세그먼트)
- 톤 변경 시 재생성, 연타 race 방지(reqId), 바이럴 루프 CTA
- **검증**: 생일 2000-03-15/이름 지호/톤 3종 모두 생성+이름 치환+캐싱 정상

### ⏭️ 다음 (작업 7)
- 카드 다운로드(html2canvas)·공유(Web Share)·/api/og (F4·F5)

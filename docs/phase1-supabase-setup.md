...
## (선택) F3.2 스티커 — universes.stickers 마이그레이션

스티커 꾸미기는 **익명(localStorage)에선 마이그레이션 없이 동작**합니다. 로그인 사용자가
스티커를 **클라우드에 동기화**하려면 [`supabase/schema.sql`](../supabase/schema.sql)의
**"v2 F3.2(스티커)"** 한 줄(`universes.stickers` 컬럼 추가)을 실행하세요(additive·재실행 안전).
미실행 시에도 스티커 **없는** 저장은 정상이며, 스티커를 붙인 항목만 클라우드 동기화에서 제외됩니다.

---

설정을 마치면 알려주세요 — 라이브 로그인·동기화·마이그레이션·목소리·스티커를 함께 검증하겠습니다.
